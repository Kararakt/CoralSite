  ...
  <div class="flex flex-col gap-[4rem] clearGap">
    <div class="container mx-auto pt-36 paddingX flex flex-col gap-6">
      <ProductLocation
        ...
        :productLocationLabel="'Remus'"
      />
      <div class="grid grid-cols-2 clearGap clearFlex">
        <div class="flex flex-col">
          <div v-for="TabRemusItem in TabRemusItems" :key="TabRemusItem.id">
            <el-image
              v-if="activeTab === TabRemusItem.count"
              :src="TabRemusItem.src"
              class="w-full h-auto"
            />
          </div>
          <div class="flex gap-4 justify-center pt-4">
            ...
            <div v-for="TabRemusItem in TabRemusItems" :key="TabRemusItem.id">
              <el-image
                @click="activeTab = TabRemusItem.count"
                :class="
                  activeTab === TabRemusItem.count ? 'activeImageBorder' : ''
                "
        ...
        <ProductInformation
          v-for="allItem in sortedItems.slice(2, 3)"
          :key="allItem.id"
          :item="allItem"
        />
      </div>
